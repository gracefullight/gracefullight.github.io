...
$$
\pi(a|s) = \begin{cases}
\text{greedy action \quad with high probability} \\
\text{random action \quad with probability $\epsilon$} \\
\end{cases}
$$

- if $\epsilon = 0.1$, then the agent will choose the greedy action with probability $0.9$ (Exploitation)
- And the random action with probability $0.1$ (Exploration).
- **Policy Improvement Theorem**:
  - for any $\epsilon$-greedy policy $\pi$
  - The $\epsilon$-greedy policy $\pi'$ is as good or better than the original policy $\pi$: $v_{\pi'}(s) \geq v_{\pi}(s)$

### Monte Carlo Control

- **Traditional Policy Iteration**:
  - Fix the policy $\pi$.
  - Evaluate the current policy until $Q$ is sufficiently close to $q_\pi(s,a)$.
...
- **Monte Carlo Control**:
  ...
  - Improve the policy using $\epsilon$-greedy after every episode.
  - $Q_1 \rightarrow \pi_1 = \epsilon\text{-greedy}(Q_1)$
  - $Q_2 \rightarrow \pi_2 = \epsilon\text{-greedy}(Q_2)$
  - $\cdots$
  - Eventually approaches $(q_*, \pi_*)$ under appropriate convergence conditions.

![MC Control](./mc-control.png)

$$
\text{Evaluate a little}
\rightarrow
\text{Improve}
\rightarrow
\text{Evaluate a little}
...
2. $\lim_{k \to \infty} \pi_k(a|s) = 1(a = \argmax_{a' \in A} Q_k(s,a'))$

- `Explore enough early -> Explore less over time -> Eventually greedy`.
  - $N_k(s,a)$: How many times the state-action pair $(s,a)$ has been visited in the $k$-th episode.
  - $\pi_k \rightarrow \text{greedy}(Q_k)$
- Infinite exploration first, greedy behavior in the limit.

### GILE Monte Carlo Control

- Algorithm flow:
  - Run episode $k = \{S_0, A_0, R_1, \dots, S_{T}\}$ with policy $\pi_k$.
  - For each state-action pair $(s,a)$ in the episode:
    - $N_k(s,a) \leftarrow N_k(s,a) + 1$
    - $Q_k(s,a) \leftarrow Q_k(s,a) + \frac{1}{N_k(s,a)} (G_k - Q_k(s,a))$
  - Improve the policy based on the new $Q$ value.
    - $\epsilon \leftarrow \frac{1}{k}$
    - $\pi \leftarrow \epsilon\text{-greedy}(Q)$
  - Repeat until convergence.
- `old estimate + step-size * (new sample - old estimate)`.
  - old estimate = $Q(S_t, A_t)$
  - new sample = $G_t$
  - step-size = $\frac{1}{N_k(s,a)}$
- Run Episode → Update Q → Reduce $\epsilon$ → Improve Policy
