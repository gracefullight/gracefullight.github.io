...
- Off-policy: Q-Learning, behavior policy is different from the target policy. $Q(S, A)$
- Update $Q(S_t, A_t)$ toward the value of the alternative, best action.

$$ Q(S_t, A_t) = Q(S_t, A_t) + \alpha [\underbrace{R_{t+1} + \gamma \max_{a} Q(S_{t+1}, a)}_{\text{TD target}} - Q(S_t, A_t)]$$

- The Actual next action is replaced by the Best possible action.

```mermaid
graph LR
  Current[S_t, A_t] --R_{t+1}--> NextState(("S_{t+1}")) -.-> a1[a1: 8]
  NextState --> a2[a2: 11]
  NextState -.-> a3[a3: 5]
```
...
- $Q(S_{t+1}, a_2)=11$
- $Q(S_{t+1}, a_3)=5$
- TD Target $=2+0.9(11)=11.9$
- TD Error $=11.9-10=1.9$
- Updated $Q(S_t, A_t)=10+0.5(1.9)=10.95$
...
![Cliff Walking](./cliff-walking.png)
...
![Cliff Walking Rewards](./cliff-walking-rewards.png)

| Aspect | SARSA | Q-Learning |
| - | - | - |
| Policy type | On-policy | Off-policy |
| Bootstrap target | $Q(S_{t+1},A_{t+1})$ | $\max_a Q(S_{t+1},a)$ |
| Next action used | Action actually taken | Best possible action |
| Learns | Value of the behavior policy being followed | Optimal policy $\pi_*$ |
| Cliff Walking | Safer path | Optimal but riskier path |
